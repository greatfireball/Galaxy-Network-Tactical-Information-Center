...
<?php
    ...
    for($b=0; $b<$alli_anzahl; $b++) {
        $aname = mysql_result($SQL_Result_Alli, $b, 'tag');
        $aticid = mysql_result($SQL_Result_Alli, $b, 'id');
        $metaid = mysql_result($SQL_Result_Alli, $b, 'ticid');
        $incs_offen = 0;
        $incs_overtime = 0;
        $incs_safe = 0;

        $SQL_Query = "SELECT ankunft, save FROM gn4accounts as a LEFT JOIN gn4flottenbewegungen as b ON(a.galaxie = b.verteidiger_galaxie AND a.ticid = b.ticid) WHERE modus = 1 AND a.ticid = ".$metaid." AND a.allianz = ".$aticid." GROUP BY b.id;";
        ...
        $SQL_Query = "SELECT count(*) FROM gn4accounts WHERE allianz = ".$aticid.";";
        $SQL_Result_alli_user = tic_mysql_query($SQL_Query) or $error_code = 43;

        $SQL_Query = "SELECT galaxie, planet, name, allianz, rang FROM gn4accounts WHERE allianz=".$aticid." AND lastlogin > ".(time() - 300)." ORDER BY galaxie, planet;";
        $SQL_Result_alli_user_online = tic_mysql_query($SQL_Query) or $error_code = 44;

        $tt_incs = "&lt;b&gt;Incs bei Allianz ".@$mname." :&lt;/b&gt;&lt;br /&gt;";
        ...
        $out_online_names = "&lt;b&gt;Online bei Allianz ".$aname." :&lt;/b&gt;&lt;br /&gt;";
        ...
        $out_allis    .= "        <td align=\"center\" width=\"75\" onmouseover=\"return overlib('&lt;b&gt;".$aname."&lt;/b&gt;');\" onmouseout=\"return nd();\">&nbsp;".($incs_offen>0?"<span class=textincopen>":"").$aname.($incs_offen>0?"</span>":"")."&nbsp;</td>\n";
