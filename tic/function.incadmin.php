<?php
    if ( strcmp( $_POST['subaction'], 'adminchange') == 0 ){
        ...
        for ( $i=0; $i<10; $i++ ){
            $sql = 'INSERT INTO `gn4vars` (ticid, name, value ) ';
            $sql .= 'VALUES("'.$Benutzer['ticid'].'", "galainc", "'.$ziel[$i].'|'.$zieldesc[$i].'" )';
            $SQL_Result = mysql_query( $sql, $SQL_DBConn);

            if ( $ziel[$i] <> 0 ){
                $sql = 'select gala, planet from `gn4incplanets`  where gala='.$ziel[$i].' and planet=1 ';
                $SQL_Result2 = mysql_query( $sql, $SQL_DBConn);
                $count = mysql_num_rows($SQL_Result2) or $count=0;
                if ( $count == 0 ){
                    for ( $j=0; $j<12; $j++ ) {
                        $nr = $j+1;
                        $sql2 = 'INSERT INTO `gn4incplanets` (ticid, planet, gala, bestaetigt, vorgemerkt ) VALUES("'.$Benutzer['ticid'].'", "'.$nr.'","'.$ziel[$i].'", "", "" )';
    ...
    } else if ( strcmp( $subaction, 'activechange') == 0 ){
        $incfreigabe=$check;
        if ( $incfreigabe=='' ) {
            $incfreigabe = 0;
        } else {
