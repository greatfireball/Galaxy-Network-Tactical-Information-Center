<center>
    ...
    <?PHP
        ...
        if ($SQL_Num == 0)
            ...
            echo '<table width="80%">';
            for ($n = 0; $n < $SQL_Num; $n++) {
                $ntype = substr(mysql_result($SQL_Result, $n, 'ticid'),0,1);
                if ($ntype=='H') {
                   $ccode = "aa3333";
                } else if ($ntype=='S') {
                   $ccode = "ff000";
                ...
                echo '      <td width="50%">'.mysql_result($SQL_Result, $n, 'titel').'</td><td width="15%">'.mysql_result($SQL_Result, $n, 'zeit').'</td><td width="35%" >'.mysql_result($SQL_Result, $n, 'name').'</td>';
                echo '  </tr>';
                echo '  <tr>';
                echo '  <td colspan="3" class="fieldnormallight" style="background-color:'.$ccode.'">'.mysql_result($SQL_Result, $n, 'text').'</td>';
                echo '  </tr>';
